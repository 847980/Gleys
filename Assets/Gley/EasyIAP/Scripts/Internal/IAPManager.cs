    ...
    public class IAPManager : MonoBehaviour, IDetailedStoreListener
    {
        ...
        public void BuyProduct(ShopProductNames productName, UnityAction<IAPOperationStatus, string, StoreProduct> OnCompleteMethod)
        {
            ...
            for (int i = 0; i < shopProducts.Count; i++)
            {
                ...
                {
                    // alternatif: buat 2 event berbeda untuk see dan buy, instead of custom params
                    // item in game yg dibeli? 
                    FirebaseManager.I.LogEvent("buy_product", new Firebase.Analytics.Parameter[]
                            { 
                                new (FirebaseAnalytics.ParameterItemName, shopProducts[i].productName),
                                new (FirebaseAnalytics.ParameterItemCategory, shopProducts[i].productType.ToString()),
                                new (FirebaseAnalytics.ParameterPrice, GetPrice(shopProducts[i].GetProductName())),
                                new ("product_action", "seek"),
                            });
            ...
            for (int i = 0; i < shopProducts.Count; i++)
            {
                if (String.Equals(e.purchasedProduct.definition.id, shopProducts[i].GetStoreID(), StringComparison.Ordinal))
                {
                    ...
                    if (validPurchase)
                    {
                        ...
                        if (!string.IsNullOrEmpty(shopProducts[i].receipt) && m_StoreController != null) ConfirmIfPending(e);
                        if (shopProducts[i].productType == ProductType.NonConsumable)
                        {
                            shopProducts[i].active = true;
                        }
                        ...
                        if (shopProducts[i].productType == ProductType.Subscription)
                        {
                            ...
                            {
                                shopProducts[i].subscriptionInfo = p.getSubscriptionInfo();
                                Debug.Log(shopProducts[i].subscriptionInfo);
                                shopProducts[i].active = shopProducts[i].subscriptionInfo.isSubscribed() == Result.True;
                            }
                            catch
                            {
                        ...
                        //Debug.Log($"proses purchase. completed. produk {shopProducts[i].productName} {shopProducts[i].productType} {shopProducts[i].rewardType}");
                        if (OnCompleteMethod != null)
                        {
                            FirebaseManager.I.LogEvent("buy_product", new Firebase.Analytics.Parameter[]
                            {
        ...
        [Serializable]
        public class GooglePlayReceiptWrapper
        {
        ...
        private void ConfirmIfPending(PurchaseEventArgs e)
        {
            var receiptWrapper = JsonUtility.FromJson<GooglePlayReceiptWrapper>(e.purchasedProduct.receipt);

            if (receiptWrapper != null)
            {
                var receipt = JsonUtility.FromJson<GooglePlayReceiptData>(receiptWrapper.Payload);
                if (receipt != null)
                {
                    if (!receipt.acknowledged) m_StoreController.ConfirmPendingPurchase(e.purchasedProduct);
                }
            }
        }
