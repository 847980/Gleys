        ...
        private void OnGUI()
        {
            ...
            if (useForGooglePlay || useForIos || useForAmazon || useForMac || useForWindows)
            {
                ...
                for (int i = 0; i < localShopProducts.Count; i++)
                {
                    ...
                    localShopProducts[i].rewardType = (RewardType)EditorGUILayout.EnumPopup("Reward Type:", localShopProducts[i].rewardType);
                    localShopProducts[i].productSo = (ProductSo)EditorGUILayout.ObjectField("Product SO:", localShopProducts[i].productSo, typeof(ProductSo), false);
