...
namespace Gley.DailyRewards.Internal
{
    public class CalendarManager : MonoBehaviour
    {
        ...
        private ValueFormatterFunction valueFormatterFunction = null;
        ...
        void Initialize()
        {
            ...
            {
                initialized = true;
                DailyRewardsData dailyRewardsSettings = Resources.Load<DailyRewardsData>(Constants.DATA_NAME_RUNTIME);
                if (dailyRewardsSettings == null)
                {
                    Debug.LogWarning("Daily Rewards is not setup. Go to Tools->Gley->Daily Rewards to setup");
                }
                else
                {
        ...
        public void SetValueFormatter(ValueFormatterFunction aFunction)
        {
            valueFormatterFunction = aFunction;
        }
        ...
        public void ShowCalendar()
        {
            if (canvas == null)
            {
                Debug.LogWarning("canvas cannot be null. Go to Tools->Gley->Daily Rewards to setup");
                return;
            }

            if (popup == null)
            {
                Debug.LogWarning("popup cannot be null. Go to Tools->Gley->Daily Rewards to setup");
                return;
            }

            if (allDays.Count == 0)
            {
                Debug.LogWarning("You need at least one day. Go to Tools->Gley->Daily Rewards to setup");
                return;
            }
        ...
        public void ButtonClick(int dayNumber, int rewardValue, Sprite rewardSprite)
        {
            ...
            if (ClickListener != null)
            {
                ...
                FirebaseManager.I.LogEvent("daily_reward", new Firebase.Analytics.Parameter[]
                {
                    new Firebase.Analytics.Parameter("day_claimed", dayNumber)
                });
            }
        }
        ...
        private void LoadPopup(GameObject canvas, GameObject popup, List<CalendarDayProperties> allDays)
        {
            ...
            calendarPopupCanvas.GetComponent<Canvas>().sortingOrder = max;
            ...
            calendarPopup.GetComponent<CalendarPopup>().Initialize(allDays, savedDay, TimeExpired(), valueFormatterFunction);
        }
    }
}